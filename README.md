...
UI using Shadcn/UI, Tailwind CSS, React, Inertia.js

## Todo

- [ ] Device scraper (https://neuraldsp.com/device-list)
  - Should scrape the device list page and convert into a JSON object
  - Should be runnable via an artisan command
  - Should be scheduled to run weekly
- [ ] Device list page (homepage)
  - A tables based list of the devices
  - Keyword filterable. The whole goal is to make keyword search fast and easy.
  - Type filterable, e.g. amps, cabs, effects, etc.


## Example working scraper

This is a scraper that gets the same content that we need to get from another project of mine. We'll want to transform
ours to PHP.
