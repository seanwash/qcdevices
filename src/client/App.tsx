                    ...
                    {table.getHeaderGroups().map((headerGroup) => (
                        <TableRow key={headerGroup.id}>
                            {headerGroup.headers.map((header) => {
                                ...
                                return (
                                    <TableHead key={header.id} className={(meta as { className?: string })?.className}>
                        ...
                        table.getRowModel().rows.map((row) => (
                            <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
                                {row.getVisibleCells().map((cell) => {
                                    ...
                                    return (
                                        <TableCell key={cell.id} className={(meta as { className?: string })?.className}>
                    ...
                    {isLoading ? (
                        <div className="rounded-lg border border-border/40 bg-card p-8 text-center shadow-sm sm:p-12">
                            <div className="space-y-4">
                                ...
                                <Typography className="text-muted-foreground">Loading devices...</Typography>
                            </div>
                        </div>
                    ) : error ? (
                        <div className="rounded-lg border border-border/40 bg-card p-8 text-center shadow-sm sm:p-12">
                            <Typography className="text-destructive">Failed to load devices. Please try again.</Typography>
                        </div>
                    ) : (
