...
function parseTable(
	$: cheerio.CheerioAPI,
	// biome-ignore lint/suspicious/noExplicitAny: cheerio type limitation
	...
	rows.each((_, row) => {
		...
		if (
			schema.addedInCorOS !== undefined &&
			cells.length > schema.addedInCorOS
		) {
		...
		if (
			schema.deviceCategory !== undefined &&
			cells.length > schema.deviceCategory
		) {
		...
		if (
			schema.previousName !== undefined &&
			cells.length > schema.previousName
		) {
		...
		if (
			schema.updatedInCorOS !== undefined &&
			cells.length > schema.updatedInCorOS
		) {
		...
		if (
			schema.pluginSource !== undefined &&
			cells.length > schema.pluginSource
		) {
